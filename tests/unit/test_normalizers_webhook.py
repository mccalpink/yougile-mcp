...
def test_webhook_filters_array_single_adds_meta_note():
    """При коррекции возвращается _meta.notes."""
    filters = [{"name": ["location"], "value": ["some-uuid"]}]
    result, meta = normalize_webhook_filters(filters, return_meta=True)
...
def test_webhook_filters_empty_list_passthrough():
    ...
    result = normalize_webhook_filters([])
    assert result == []
...
def test_webhook_filters_idempotent():
    ...
    r1 = normalize_webhook_filters(filters)
    r2 = normalize_webhook_filters(r1)
